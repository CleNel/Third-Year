...
unsigned long hash_function_2(const char *s){
...
void insert(char* name){
   // TODO3
	...
	int currentIndex = hash_function_2(name);
...
void printNumberOfOccurences(char* name){
    //TODO5
		...
		if(search(name) == NULL){
			...
			printf("%s - %i ",name,hashTable[hash_function_2(name)] -> freq);
		}
}
...
void addOrIncrement(char* name){
	...
	if(search(name) == NULL){
		...
		hashTable[hash_function_2(name)] -> freq++;
	}
}
...
int main ( int argc, char *argv[] ) {
    ...
    load_file("names.csv");
	//printf("Cle is %u\n", hash_function_2("Cleo"));

	//printf("(%i)\n",hash_function_2("Wicken"));
